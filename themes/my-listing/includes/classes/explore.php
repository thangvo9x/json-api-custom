...
class Explore {
	...
	public static function init() {
		add_action( 'init', [ '\CASE27\Classes\Explore', 'add_rewrite_rules' ] );
	}
	...
	public function parse_active_category() {
		...
		if ( get_query_var( 'explore_category' ) && ( $term = get_term_by( 'slug', sanitize_title( get_query_var( 'explore_category' ) ), 'job_listing_category' ) ) ) {
	...
	public static function add_rewrite_rules() {
	    ...
	    // Add rewrite tags.
    	add_rewrite_tag( '%explore_category%', '([a-zA-Z\d\-_+]+)' );
    	add_rewrite_tag( '%explore_region%', '([a-zA-Z\d\-_+]+)' );
    	add_rewrite_tag( '%explore_tag%', '([a-zA-Z\d\-_+]+)' );
